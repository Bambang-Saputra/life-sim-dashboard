...
- **Living pixel scenes** — each page hero is an interactive scene: mouse parallax on three depth layers, a trophy that bursts into sparkles, a vault that shakes and pops a coin, a TV that turns on, books that lift on hover
...
Both jobs are idempotent and also fire as a safety net when the relevant page is opened — nothing breaks if the scheduler never runs.
...
- **Livewire manual bundle** (`livewire.esm` + `Livewire.start()`) so Alpine components register before the DOM walk — no double-Alpine bugs
